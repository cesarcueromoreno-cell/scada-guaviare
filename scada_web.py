...
def simular_historico_24h_avanzado(planta):
    ...
    for m in range(0, 24 * 60, 15):
        ...
        datos.append({
            "timestamp": t,
            "Potencia solar": round(gen * 1000, 2), # En W
            "Consumo": round(con * 1000, 2),        # En W
            "Red": round(grid_purchase * 1000, 2)   # En W
        })
...
if menu == "👥 Gestión de Usuarios":
    ...
    with t_graf:
        ...
        with col_main:
            ...
            # Leyenda falsa para simular "Clima" (Gris claro)
        ...
        with col_bot_r:
            ...
            # Cabecera de Producción Planificada simulando la interfaz de Solarman
            st.markdown("""
            <div style="display: flex; justify-content: space-between; align-items: center; margin-bottom: 10px;">
                <h4 style="margin: 0; color: #2c3e50; font-size: 15px; font-weight: normal;">Producción Planificada</h4>
                <div style="display: flex; gap: 5px; align-items: center;">
                    <button style="border:1px solid #3498db; background:white; color:#3498db; padding:2px 8px; border-radius:4px; font-size:12px;">Año</button>
                    <button style="border:1px solid #eaeaea; background:white; color:#7f8c8d; padding:2px 8px; border-radius:4px; font-size:12px;">Total</button>
                    <button style="border:none; background:#1890ff; color:white; padding:2px 8px; border-radius:4px; font-size:12px;">Exportar</button>
                    <span style="color:#7f8c8d; margin-left:10px;">&lt;</span>
                    <span style="border:1px solid #eaeaea; padding:2px 10px; border-radius:4px; font-size:12px; color:#2c3e50;">2025 📅</span>
            ...
            # Trazos invisibles grises para generar la leyenda pedida en la captura
            fig2.add_trace(go.Bar(x=df_mensual['Mes'], y=[0]*12, name='Producción Planificada Mensual', marker_color='#d9d9d9', width=0.4))
            fig2.add_trace(go.Bar(x=df_mensual['Mes'], y=[0]*12, name='Finalización mensual', marker_color='#bfbfbf', width=0.4))

            fig2.update_layout(
                paper_bgcolor="rgba(0,0,0,0)", plot_bgcolor="rgba(0,0,0,0)", 
                barmode='group',
                legend=dict(orientation="h", yanchor="bottom", y=-0.3, xanchor="center", x=0.5, font=dict(size=12, color="#7f8c8d")),
        ...
        # --- FILA ULTRA-INFERIOR: RESUMEN DEL SISTEMA ---
        st.markdown("<div style='height: 15px;'></div>", unsafe_allow_html=True)
        st.markdown("""
