...
:root {
    ...
    --text-color: #2c3e50;
    --accent-color: #f39c12; /* Naranja/Amarillo acento CV */
    --border-color: #e0e0e0;
}
...
/* Forzamos que la block-container (el panel central) sea blanca y nítida */
.block-container { 
...
# ==========================================
# 2. BLINDAJE DE VARIABLES DE SESIÓN Y LOGIN
# ==========================================
...
def cargar_usuarios():
    if not os.path.exists('usuarios.json'): 
        # Usuario admin por defecto para CV Ingeniería
...
menu = st.sidebar.radio("IR A:", ["🌐 Panorama General", "📊 Panel de Planta", "🚨 Alarmas"], key="nav_main")

# ARREGLO SEGURO DEL BOTÓN CERRAR SESIÓN (Sin TypeError)
...
if menu == "🌐 Panorama General":
    ...
    if not plantas: st.info("No hay plantas registradas."); st.stop()
    ...
    for p in filtered:
        ...
        if c1.button("Ver Detalle", key=f"panel_{p['nombre']}", use_container_width=True): 
            # Guardamos la planta seleccionada en session state y cambiamos de menú
            st.session_state["planta_seleccionada"] = p['nombre']
            # Para cambiar el radio button, necesitamos trucos o session state. Lo más fácil:
            # Reemplazar el session state del nav_main y reruntear
            st.session_state["nav_main"] = "📊 Panel de Planta"
            st.rerun()

# ==========================================
# 5. VISTA: PANEL DE PLANTA (EL PLATO FUERTE)
# ==========================================
elif menu == "📊 Panel de Planta":
    plantas = cargar_plantas()
    if not plantas: st.info("No hay plantas registradas. Agregue una."); st.stop()
    
    # Recuperar selección automática si viene del panorama
    planta_default_index = 0
    if "planta_seleccionada" in st.session_state:
        try:
            planta_default_index = [p['nombre'] for p in plantas].index(st.session_state["planta_seleccionada"])
        except: planta_default_index = 0

    col_sel, _ = st.columns([1.5, 2])
    planta_sel = col_sel.selectbox("📋 Seleccionar planta activa:", [p['nombre'] for p in plantas], index=planta_default_index, label_visibility="collapsed")
    ...
    with t_graf:
        # ------------------------------------------------------------------------------------------------
        # EL DIAGRAMA DE FLUJO "REAL, BONITO Y QUE SE MUEVE" (SVG HD ANIMADO)
        # ------------------------------------------------------------------------------------------------
        ...
        # Batería (d['bat'] < 0 significa descargando -> flujo hacia inversor)
        bat_inverter = "descargando" if d['bat'] < 0 else "cargando" if d['bat'] > 0 else "idle"
        # Red (d['red'] < 0 significa exportando -> flujo hacia red)
        inverter_red = "exportando" if d['red'] < 0 else "importando" if d['red'] > 0 else "idle"
        
        # Definición de velocidades de puntos (segundos por ciclo). Menos segundos = más rápido.
    ...
    # --- CONTROL DE RED (PRESERVADO INTACTO SEGÚN REQUERIMIENTO) ---
    if t_red:
        with t_red:
            ...
            if not st.session_state["red_desbloqueada"]:
                ...
                ct3.empty() # Sin tiempo específico para inyección
    ...
    # --- O&M (AGENDA DE MANTENIMIENTO PRESERVADO INTACTO) ---
    if t_om:
        with t_om:
            st.markdown("### 📅 Agenda de Mantenimiento / Reportes")
            # Simulación descarga reporte
            ...
            if not mantenimientos: st.info("No hay mantenimientos programados.")
                ...
                for i, m in enumerate(reversed(mantenimientos)):
                    ...
                    st.markdown(f"<div style='background:white; padding:15px; border-radius:8px; border:1px solid #eaeaea; margin-bottom:10px; color:#2c3e50;'><b>{m['tipo']}</b> - {m['estado']}<br><span style='font-size:12px; color:#7f8c8d;'>📅 {m['fecha']} | 👨‍🔧 {m['resp']} | 📝 {m['notas']}</span></div>", unsafe_allow_html=True)
    ...
    st.title("🚨 ALERTAS")
    # Simulación de alertas inteligentes
    plantas = cargar_plantas()
    if not plantas: st.info("No hay plantas registradas.")
    else:
        for p in plantas:
            # Una de cada 3 plantas simuladas tiene alerta
