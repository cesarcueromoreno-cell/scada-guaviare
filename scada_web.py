...
# Si no está autenticado, mostramos la pantalla de Login y DETENEMOS el resto del código
if not st.session_state["autenticado"]:
    st.markdown("<h1 style='text-align: center;'>🔒 Acceso Seguro - SCADA</h1>", unsafe_allow_html=True)
    st.markdown("<p style='text-align: center;'>Ingrese sus credenciales para acceder al panel de control de la flota.</p>", unsafe_allow_html=True)
    
    st.markdown("---")
    col_vacia1, col_centro, col_vacia2 = st.columns([1, 2, 1]) # Centramos el formulario
    ...
    with col_centro:
        with st.form("login_form"):
            ...
            if submit_login:
                # Aquí defines tu usuario y contraseña maestra
                if usuario == "admin" and contrasena == "solar123":
                    st.session_state["autenticado"] = True
                    st.rerun() # Recargamos la página para que lo deje entrar
                else:
                    st.error("❌ Usuario o contraseña incorrectos.")
    
    st.stop() # ¡MAGIA! Esto evita que el código de abajo se ejecute si no hay login.
